...
| -------------- | ------------- | -------- | ------ |
| 0.9.0          | 1.5.x / 1.4.x | AWP 1  | Supported, recommended — pairs with the AuraDB v1.5.0 line. Adds live analyzer and snippet search ergonomics for AuraDB: `AnalyzerOptions`, `search_text(..., analyzer=...)`, a `.analyzer(...)` builder method, hybrid analyzer requests, snippet request helpers with typed `SearchSnippet` / `SearchSnippetFragment` / `HighlightRange` result models (byte-to-character range handling for Python slicing), and analyzer-aware `aura.search_quality` report parsing. **No protocol change, no new wire feature, no breaking API change**; existing `connect(...)` / `Client(...)` constructors are unchanged and it keeps working against v1.4.x/v1.3.x servers. The AuraDB-only analyzer/snippet paths are capability-gated on the server's `query_analyzers` / `search_snippets` capabilities and raise `AuraCapabilityError` against a server that does not advertise them. |
| 0.8.0          | 1.4.x / 1.3.x | AWP 1  | Supported — pairs with the AuraDB v1.4.0 line. Adds client-side ergonomics only: environment-driven connection profiles (`ConnectionProfile` / `from_env` / `from_profile`, TLS CA and SNI/`server_name` wiring, token-redacting `repr`), capability UX (`capabilities().require(...)` / `.describe()`), and typed parsers for AuraDB's `search eval` reports (`aura.search_quality`: `SearchEvalReport` / `SearchEvalMetrics` / `SearchEvalQueryResult` and BM25/hybrid models). **No protocol change, no new wire feature, no breaking API change**; existing `connect(...)` / `Client(...)` constructors are unchanged and it keeps working against v1.3.x servers. The relevance reports are produced by the AuraDB CLI and parsed client-side; the connector does not run server-side CLI commands. |
...
- **The 0.9.0 analyzer/snippet ergonomics are additive and capability-gated.** They require an
  AuraDB **v1.5+** server that advertises `query_analyzers` / `search_snippets`; against an
  older server or a non-AuraDB backend the AuraDB-only search paths raise `AuraCapabilityError`
  rather than silently degrading. `english_basic` is a small, deterministic analyzer — not full
  NLP — and the parsed relevance scores are fixture-specific regression signals, not universal
  benchmarks.
